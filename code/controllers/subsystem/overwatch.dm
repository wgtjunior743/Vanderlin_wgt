...
/datum/controller/subsystem/overwatch/proc/CheckASNban(client/C)
	...
	var/datum/DBQuery/query = SSdbcore.NewQuery("SELECT `asn` FROM overwatch_asn_ban WHERE asn = :asn", list("asn" = C.ip_info.ip_as))
	query.Execute()
...
/datum/controller/subsystem/overwatch/proc/LoadCachedData(ip)
	...
	var/datum/DBQuery/_Cache_select_query = SSdbcore.NewQuery("SELECT response FROM overwatch_ip_cache WHERE ip = :ip", list("ip" = ip))
...
/datum/controller/subsystem/overwatch/proc/CacheData(ip, raw_response)
	...
	var/datum/DBQuery/_Cache_insert_query = SSdbcore.NewQuery({"
...
/datum/controller/subsystem/overwatch/proc/AddToWhitelist(ckey_input, client/Admin)
	...
	var/datum/DBQuery/_Whitelist_Query = SSdbcore.NewQuery("INSERT INTO overwatch_whitelist (`ckey`, `a_ckey`, `timestamp`) VALUES (:ckey, :admin_ckey, Now())", list("ckey" = ckey, "admin_ckey" = Admin.ckey))
...
/datum/controller/subsystem/overwatch/proc/RemoveFromWhitelist(ckey, client/Admin)
	...
	var/datum/DBQuery/_Whitelist_Query = SSdbcore.NewQuery("DELETE FROM overwatch_whitelist WHERE `ckey` = :ckey", list("ckey" = ckey))
	_Whitelist_Query.Execute()
	qdel(_Whitelist_Query)
...
/datum/controller/subsystem/overwatch/proc/GetWhitelistDatabase()
	var/datum/DBQuery/_Whitelist_DB_Select_Query = SSdbcore.NewQuery("SELECT `ckey`, `a_ckey`, `timestamp` from overwatch_whitelist")
...
/datum/controller/subsystem/overwatch/proc/AddASNban(address, client/Admin)
	...
	var/datum/DBQuery/_ASban_Insert_Query = SSdbcore.NewQuery(
...
/datum/controller/subsystem/overwatch/proc/RemoveASNban(ip_as, client/Admin)
	...
	var/datum/DBQuery/_ASban_Delete_Query = SSdbcore.NewQuery("DELETE FROM overwatch_asn_ban WHERE `asn` = :asn", list("asn" = ip_as))
...
/datum/controller/subsystem/overwatch/proc/GetAsnBanlistDatabase()
	var/datum/DBQuery/_ASN_Banlist_Select_Query = SSdbcore.NewQuery("SELECT `asn`, `timestamp`, `a_ckey` from overwatch_asn_ban")
...
/datum/controller/subsystem/overwatch/proc/HandleClientAccessCheck(client/C, postponed = 0)
	if(!SSoverwatch.CheckForAccess(C) && !(C.ckey in GLOB.admin_datums))
		...
		message_admins(span_notice("Overwatch: Failed Login: [C.key]/[C.ckey]([C.address])([C.computer_id]) failed to pass Overwatch check."))
		//qdel(C)
		return TRUE
	return FALSE
