...
/datum/controller/subsystem/processing/fishing/Initialize()
	...
	var/icon/questionmark = icon('icons/obj/structures_spawners.dmi', "map_spawner")
	...
	for(var/obj/item/reagent_containers/food/snacks/fish/fish_type as anything in fish_subtypes)
		...
		var/obj/item/reagent_containers/food/snacks/fish/fish = new fish_type
		spawned_fish += fish
		...
		if(fish.fish_id_redirect_path)
			...
			if(!ispath(other_path, /obj/item/reagent_containers/food/snacks/fish))
	...
	for(var/lure_type in subtypesof(/obj/item/fishing/lure))
		var/obj/item/fishing/lure/lure = new lure_type
		lure_catchables[lure_type] = list()
		for(var/obj/item/reagent_containers/food/snacks/fish/fish as anything in spawned_fish)
